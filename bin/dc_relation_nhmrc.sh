...
from
  ...
  (
    select item_id from collection2item
    where item_id not in (select item_id from item where withdrawn=true) and
      collection_id =
       (select collection_id from collection where name='National Health and Medical Research Council (NHMRC)')
  ) i
where
  mv.item_id = i.item_id and
  mv.metadata_field_id = mfr.metadata_field_id and
  mfr.element = 'relation' and
  $where_subclause
order by dc_value, item_id
"
